...
namespace neuro_local_planner_wrapper
{
                                         ...
                                         costmap_2d::Costmap2DROS* costmap_ros)
    {
        ...
        if (!initialized_)
        {
            ...
            // Init counters for reached goals and crashes during one training run
            goal_counter_ = 0;
            crash_counter_ = 0;
    ...
    bool NeuroLocalPlannerWrapper::isCrashed(double& reward)
    {
        ...
        if(costmap_->getCost((unsigned int)robot_x, (unsigned int)robot_y) >= 200)
        {
            crash_counter_++;
